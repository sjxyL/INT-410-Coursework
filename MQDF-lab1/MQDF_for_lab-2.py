...
def QDF_model(train_data, class_num):  # Modified function from QDF to obtain the required parameters for MQDF
    ...
    for i in range(class_num):  # For all classes
        ...
        train_data[i] = np.mat(train_data[i], dtype=np.float64) # Convert the ith data to a matrix & use 64-bit precision
...
def MQDF2_model(cov, d, k, class_num):  # Function to obtain the trained parameters of MQDF2
    ...
    for i in range(class_num):  # For all classes
        ...
        eigvals, eigvecs = np.linalg.eigh(covMat)# Obtain eigenvalues and eigenvectors from the cov. matrix. Tip: usenp.linalg.eig()
        ...
        eigvecs = eigvecs = eigvecs[:, id] # Descending the eigenvectors
        ...
        delta[i] = np.sum(eigvals[k:]) / (d - k) if d-k > 0 else 1 # Compute delta as the mean of minor values. Tip: sum(eigvals[int(k):]) / (d - k)
        ###############################################################################################################
...
def predict_MQDF2(d, np_x, class_num, k, mean, eigenvalue, eigenvector, delta):
    pred_label = []
    for sample in np_x:
        test_x = np.matrix(sample, np.float64).T
        max_g2 = -float('inf')
...
if __name__ == '__main__':
    ...
    ###############################################################################################################
    #                                   YOU NEED FILL FOLLOWING CODES:
    for h in range(100):# fill here): # find a range of h for test
    ###############################################################################################################
        mqdf_sum_avg_acc = 0
        cnt = 1
        for index in range(len(five_data)): # 5-fold cross validation
            ...
            #train = [np_se_num, np_ver_num, np_vir_num ]
    ...
    ###############################################################################################################
    #                                   YOU NEED FILL FOLLOWING CODES:
    for k in range(d+1): # find a range of k for test
    ##############################################################################################################
        mqdf_sum_avg_acc = 0
        cnt = 1
        for index in range(len(five_data)): # 5-fold cross validation
